...
public class RejectQueueHandler extends Handler {
    ...
    private static final String NOT_REJECT_INCIDENT = "notRejectIncident";
    ...
    public String getLabel(User user, Update update) {
        return new StringBuilder().append(incidentService.get(IncidentType.REJECT).size()).append(" | ")
    ...
    public List<TelegramResponse> getMessage(User user, Update update) {
        ...
        if (incidents == null || incidents.size() == 0) {
            ...
            return Arrays.asList(new TelegramResponse(new AnswerCallbackQuery()
                    .setText(answerService.get(NOT_REJECT_INCIDENT, user.getLanguage()).getText())
    ...
    public List<TelegramResponse> handle(User user, Update update) {
        ...
        if (method.equals(REJECT_BUTTON)) {
            incident.setType(IncidentType.DELETE);
        }
